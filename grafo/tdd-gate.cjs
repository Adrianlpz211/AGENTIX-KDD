...
function runSelfHealingLoop(opts) {
  ...
  while (iteration < MAX_HEALING_ITERATIONS) {
    ...
    if (result.allPassed) {
      ...
      // ── POST-CYCLE AUTOMÁTICO ─────────────────────────────────────────────
      // Esto es código determinístico — no depende del LLM.
      // Se ejecuta en cada PASS sin importar si el agente lo instruyó o no.
      try {
        const postCyclePath = require('path').join(__dirname, 'post-cycle.cjs');
        const fs = require('fs');
        if (fs.existsSync(postCyclePath)) {
          require('child_process').execSync(
            `node "${postCyclePath}" ${area || 'global'} --tests=${result.passed} --total=${result.total} --task="${area || 'global'}" --silent`,
            { cwd: projectRoot || process.cwd(), stdio: 'pipe', timeout: 60000 }
          );
          console.log('[TDD-GATE] ⚙️  Post-cycle completado — ciclo registrado, contratos actualizados, specs generadas');
        }
      } catch(e) {
        // post-cycle es best-effort — nunca bloquea el PASS
        console.log('[TDD-GATE] ⚠️  Post-cycle: ' + (e.message || '').slice(0, 80));
      }
